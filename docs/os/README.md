...
Guide for configuring Ubuntu 24.04 LTS as an AI engineering workstation for running large language models and AI workloads on the AMD Ryzen 9950X + RTX 5090 hardware configuration.

This documentation covers system setup from OS installation through GPU acceleration, containerization, and Python environment configuration. The setup supports both GPU-accelerated inference (models in VRAM) and CPU-based model execution for running multiple models simultaneously.

**Note**: This guide references automation scripts in `scripts/` and configuration files. Review all scripts before execution and adapt them to your requirements and security policies.
...
The AI engineering workstation implementation uses automation designed for the AMD Ryzen 9950X + RTX 5090 hardware configuration. The setup targets AI workload performance while maintaining system security and development flexibility.

Key Implementation Features:
- **Hardware optimization**: AMD Zen 5 and NVIDIA Blackwell architecture targeting
- **Automated setup**: Comprehensive script collection for consistent environment setup
...
- **RAM**: 128GB DDR5 EXPO for large model loading and multi-model inference
...
Ubuntu 24.04 LTS provides hardware support for NVIDIA RTX 5090 Blackwell architecture and AMD Ryzen 9950X Zen 5 processors for AI development workstations.

System Installation Process:
- Install Ubuntu 24.04 LTS on Samsung 990 Pro 2TB SSD
- Configure UEFI/BIOS settings (see `docs/bios/README.md`)
- Enable hardware acceleration and memory configurations
- Set up user account with administrative privileges
...
Firefox provides privacy controls, extensions, and developer tools for AI development workflows, including model repository access, documentation research, and API testing.

Browser Setup Process:
- Install Firefox browser
- Install extensions: uBlock Origin, Privacy Badger, Developer tools
- Configure privacy settings
- Set up bookmark synchronization
- Configure security for GitHub and AI platforms

### Git and GitHub Integration

Version Control for AI Development
Git integration provides version control for AI projects, model versioning, experiment tracking, and collaborative development.

Git Configuration Process:
- Configure Git with user credentials and commit signing
- Generate SSH keys for GitHub authentication
- Set up GPG signing for commit verification
- Configure backup of configuration files to repositories
- Establish repository structure for AI projects
...
Visual Studio Code supports AI/ML development with Python, Jupyter notebooks, Docker integration, and extensions for model development.

VS Code Setup Process:
- Install VS Code with AI/ML extensions
- Configure Python interpreter with pyenv
- Set up Jupyter notebook support
- Install Docker extensions
- Configure Git integration with SSH keys
...
```
- Create AI development user with sudo privileges
- Configure user groups for GPU access (video, docker groups)
- Set up file permissions for model storage and cache directories
- Implement security controls with development flexibility
- Configure shell environment and tool access
...
Terminal environment configured for AI development workflows, model management, and system administration tasks across multiple sessions.
...
```
- Install Zsh shell with customizations
- Set up Tilix terminal emulator with multi-pane support
- Install development packages: git, curl, wget, htop, nvtop, build tools
- Create aliases for Docker commands, model management, and GPU monitoring
...
- Model management: Download, conversion, and inference automation
...
Security implementation protects AI models, data, and development environment from threats while preserving performance for AI workloads and development flexibility.
...
Security Components:
- **Automated Security Updates**: Unattended-upgrades with scheduled 3 AM reboots
- **AI Development Firewall**: UFW allowing Jupyter (8888), TensorBoard (6006), Gradio (7860), Streamlit (8501), and Docker ports
- **Intrusion Prevention**: Fail2Ban with server protection rules
- **System Auditing**: Logging for file access and authentication events
- **Kernel Security**: Security parameters including ASLR, SYN flood protection, IP spoofing prevention
- **AI Resource Limits**: System limits for model loading (unlimited memory lock, 65,000 file descriptors, 32,000 processes)
- **Daily Security Scanning**: Rootkit detection and system integrity verification
- **Container Security**: Docker networking security
- **AppArmor Integration**: Ubuntu access controls for process isolation

AI Development Security Features:
- Jupyter notebook security with token authentication
- Model repository access controls
- GPU resource access controls
...
```
- Mount Samsung 990 EVO 1TB SSD at `/mnt/ai-data`
- Optimize file system for large model files (>10GB)
- Configure storage monitoring with disk usage alerts
- Set up automated cleanup for temporary files and cache
- Implement SSD health monitoring
...
System analysis tools provide information for AI model inference, resource management, and troubleshooting across the hardware and software stack.

System Analysis Process:
```bash
# System analysis and dependency verification
...
- **Hardware Analysis**: CPU features, GPU compute capability, memory
- **Software Stack**: NVIDIA drivers, CUDA toolkit, cuDNN versions
- **Python Ecosystem**: PyTorch, TensorFlow, Transformers versions
- **Container Environment**: Docker, NVIDIA Container Toolkit status
- **Performance Metrics**: Memory bandwidth, GPU utilization, storage throughput

System Monitoring Integration:
- Hardware monitoring with `nvtop`, `htop`
- Health checks for AI development components
...
```
- Install NVIDIA drivers (version 570+) for RTX 5090
- Configure driver persistence and power management
- Verify GPU detection, compute capability (sm_120), and VRAM (32GB)
- Set up driver update monitoring
- Configure X11/Wayland integration
...
cuDNN provides hardware-accelerated deep neural network primitives for AI model performance, with RTX 5090 optimizations and Blackwell architecture support.
...
Docker enables AI model inference and dependency management for local development. The NVIDIA Container Toolkit provides GPU access for containerized AI inference and training workloads.
...
```
- Install Docker CE with user permissions and GPU access
- Configure Docker daemon for AI containers
- Install NVIDIA Container Toolkit
- Set up Docker Compose for multi-container management
...
The implementation supports the containerized AI model inference documented in `docs/sandbox/`:
...
```yaml
# Multi-container AI inference
docker-compose up -d
```

Reference Files:
- `scripts/setup/setup_docker.sh`: Docker and NVIDIA Container Toolkit installation
- `docker-compose.yaml`: Multi-container AI inference management
...
Pyenv provides Python version management for AI development with different framework requirements. Poetry offers dependency management and virtual environment handling for AI projects and large framework installations.
...
- **Optimization**: deepspeed for multi-GPU training and inference
...
Backup strategy protects AI development environment, model data, experimental results, and system configurations against hardware failure, data corruption, and accidental deletion.

Backup Implementation Strategy:
- Configure Timeshift for system snapshots
- Set up external backup storage for models and results
- Create backup scripts for models and datasets
- Implement backup verification and recovery testing
...
- `docker-compose.yaml`: Containerized AI inference management
...
- **Docker Containers**: Containerized AI inference in `docker/`
